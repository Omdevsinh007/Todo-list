...
const detail = document.getElementById("addTaskDetail");
...
const updateTaskDetail = document.getElementById("updateTaskDetail");
const loader = document.getElementById("loader");
const loaderMain = document.getElementById("loader");
const WebTheme = localStorage.getItem("WebTheme") || "Light";
...
addData.addEventListener("click", () => {
    ...
    let taskDetails = detail.value.trim();
...
function displayTask(name) {
    ...
    if (userTasks.length == 0) {
        main.textContent += "No tasks to do in current moment.";
    }
    else {
...
function update(i) {
    if (updateTask.value == '' && updateTaskDetail.value == '') {
        ...
        let targetDiv = document.querySelector(`#div${i}`);
...
search.addEventListener('keyup', () => {
    ...
    setTimeout(() => {
      ...
      `;
        ...
        if (arr.length === 0) {
            main.innerHTML = '<h3>No such tasks at this moment</h3>';
        }
        loader.style.display = 'none';
    }, 500);
});
